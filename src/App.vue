<template>
  <theme-provider :theme="theme">
    ...
    <card-container id="vision">
      ...
      />
      <minor-card-container id="values">
        <minor-card
          v-for="(product, index) in products.minor && products.minor.slice(0,4)" :key="index"
          v-bind:name="product.name"
          v-bind:description="product.description"
          v-bind:link="product.link"
        />
      </minor-card-container>
      <logo-section-container id="internships">
        <main-title>{{logoSection.title}}</main-title>
        <logo-container>
          <logo-card
            v-for="(logo, index) in logoSection.logos && logoSection.logos.slice(0,5)" :key="index"
            v-bind:src="logo.src"
            v-bind:url="logo.url"
            v-bind:alt="logo.alt"
          />
